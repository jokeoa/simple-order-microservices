...
func (r *Repository) Create(ctx context.Context, order domain.Order) (domain.Order, error) {
	const query = `
		INSERT INTO orders (id, customer_id, item_name, amount, status, payment_transaction_id)
		VALUES ($1, $2, $3, $4, $5, NULLIF($6, ''))
		RETURNING id, customer_id, item_name, amount, status, COALESCE(payment_transaction_id, ''), created_at, updated_at
	`

	var created domain.Order
	if err := r.pool.QueryRow(ctx, query, order.ID, order.CustomerID, order.ItemName, order.Amount, order.Status, order.PaymentTransactionID).Scan(
		&created.ID,
		&created.CustomerID,
		&created.ItemName,
		&created.Amount,
...
func (r *Repository) GetByID(ctx context.Context, id string) (domain.Order, error) {
	const query = `
		SELECT id, customer_id, item_name, amount, status, COALESCE(payment_transaction_id, ''), created_at, updated_at
		FROM orders
		WHERE id = $1
	`
